class FlightBookingSystem {
  ...
  showBookings(criteria) {
    ...
    if (criteria === 'all') {
      const bookingsList = this.bookings.map(
        (booking) => `${booking.passengerName} booked for flight ${booking.flightNumber}.`
      );
      return ['All bookings:', ...bookingsList];
    } else if (criteria === 'cheap' || criteria === 'expensive') {
      const priceLimit = criteria === 'cheap' ? 100 : 100;

      const cheapOrExpensiveBookings = this.bookings.filter(
        (booking) => {
          const flight = this.flights.find((f) => f.flightNumber === booking.flightNumber);
          return flight && flight.price <= priceLimit;
        }
      );

      if (cheapOrExpensiveBookings.length === 0) {
        return `No ${criteria} bookings found.`;
      }

      const bookingsList = cheapOrExpensiveBookings.map(
        (booking) => `${booking.passengerName} booked for flight ${booking.flightNumber}.`
      );

      return [`${criteria.charAt(0).toUpperCase() + criteria.slice(1)} bookings:`, ...bookingsList];
    } else {
